...
# add shortest path
#with open("eig_imp.txt", "r") as f:
#        reader = csv.reader(f)
#        eig_imp  = list(reader)

#eig_imp = [float(i[0]) for i in eig_imp]
#a = np.array(eig_imp)[:,np.newaxis]
#b = trainX
#trainXX = np.append(a,b,axis=1)

# the data, shuffled and split between train and test sets
# convert class vectors to binary class matrices
X_train, X_test, y_train, y_test = cross_validation.train_test_split(trainXX,trainy, 
